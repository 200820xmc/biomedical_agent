...
> **Archive / Legacy**：记录修复前问题与分批整改过程，不代表当前未解决问题清单。
...
> 以下均为2026-07-22清空重建之前的历史评测证据。当前68篇索引尚未重跑Full或BL-1。
...
## 1.2 2026-07-22索引维护结果

- 永久删除47篇未入库PDF及其失败任务记录，不再尝试录入；原69个解析目录中另有1篇空Markdown随之移除，因此当前有效语料为68篇。
- 先清空Milvus `biz` 的1953行旧实体，再复用68份现有Markdown全量写入1305个Chunk；未调用xParse。
- 初次重建后发现2组不同PDF文件实际对应同一论文：`doc_49476e`/`doc_edffea` 与 `doc_26b3f0`/`doc_2954af`。经用户确认，删除解析内容较少的 `doc_49476e`、`doc_26b3f0` 及21个Chunk，保留 `doc_edffea`、`doc_2954af`。
- 最终全库审计确认66个本地来源与66个Milvus `source_id` 一一对应，共1284个Chunk；元数据、正文哈希、逻辑Chunk ID、文内连续序号均正常，同源及跨来源正文重复为0，待入库列表为0。
- Full、BL-1和Ragas均未重跑；2026-07-17指标只能作为重建前历史结果。

> 本文档根据当前工作区代码进行静态检查，不代表所有改动已经通过运行测试、集成测试和真实问答回归。

## 1.3 2026-07-23评测集与Top-K更新

- 生产检索基础证据Top-K由4调整为5；Dense候选20、Rerank保留10和阈值保底Top-3不变。
- 已生成当前66篇索引对应的v2候选评测集：50题全部映射，2个已删除来源迁移到保留来源；模型辅助清理将143个自动候选缩减为91个，删除52个假Gold，清理后25题具有2—4个候选可接受Chunk。
- v2使用主张级多Gold，同一主张的摘要与正文证据可作为OR替代；不会因相邻或同源就自动把Chunk标为Gold。
- 参考主张和可接受Chunk仍待人工复核，不能用于发布正式Context Recall。
- Full与BL-1脚本已加入二值Recall@3、二值Recall@5和MRR，MRR按 `1 / 首个正确名次`；同时已过滤非有限Ragas分数并保存完整回答和上下文。
- 当前索引仍未运行Full、BL-1或Ragas，旧指标继续只作历史证据。
